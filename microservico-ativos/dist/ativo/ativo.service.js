...
let AtivoService = class AtivoService {
    constructor(ativoRepository) {
        ...
        this.mongoRepo = this.ativoRepository.manager.getMongoRepository(ativo_entity_1.Ativo);
    }
    ...
    async findOne(id) {
        ...
        if (!ativo)
            throw new common_1.NotFoundException(`Ativo ${id} não encontrado`);
        return ativo;
    }
    async update(id, updateAtivoDto) {
        const objectId = new mongodb_1.ObjectId(id);
        const ativo = await this.mongoRepo.findOne({ where: { _id: objectId } });
        if (!ativo)
            throw new common_1.NotFoundException(`Ativo ${id} não encontrado`);
